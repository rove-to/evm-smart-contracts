...
contract RockNFTFor721 is ERC1155Tradable {
    ...
    public payable
    {
        ...
        require(_erc721Add != address(0x0), "MISS_ERC721_TOKEN");
        ...
        // check user mint fee
        if (price_tokens[_id] > 0 || metaverseNFTCollRockPrice[_metaverseId] > 0) {
            ParameterControl _p = ParameterControl(parameterControlAdd);
            uint256 purchaseFeePercent = _p.getUInt256("ROCK_721_PUR_FEE");
    ...
    external payable
    {
        ...
        ParameterControl _p = ParameterControl(parameterControlAdd);
        // get fee for imo
        uint256 imoFEE = _p.getUInt256("INIT_721_IMO_FEE");
        if (imoFEE > 0) {
            require(msg.value >= imoFEE * rockIdNFTCollsSize, "MISS_INI_FEE");
        }
