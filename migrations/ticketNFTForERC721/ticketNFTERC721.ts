...
class TicketNFTERC721 {
    ...
    async deploy(adminAddress: any, operatorAddress: any, paramAddress: any) {
        ...
        const NFTDeploy = await TicketNFT.deploy(adminAddress, operatorAddress, paramAddress);

        console.log("Rove Ticket NFT deployed:", NFTDeploy.address);
    ...
    async publishTicketNFT(initOwnerAddress: any, contractAddress: any, erc721: any, initSupply: number, price: string, max: number, tokenURI: string, gas: number) {
        let temp = this.getContract(contractAddress);
        const nonce = await temp?.web3.eth.getTransactionCount(this.senderPublicKey, "latest") //get latest nonce

        const eth = ethers.utils.parseEther(price);
        const fun = temp?.nftContract.methods.publishTicket(initOwnerAddress, erc721, initSupply, tokenURI, eth, max);
        console.log(gas);
        //the transaction
        const tx = {
            from: this.senderPublicKey,
    ...
    async userMintTicketNFT(to: any, contractAddress: any, tokenId: number, amount: number, erc721TokenId: number, ethAmount: string, gas: number) {
        let temp = this.getContract(contractAddress);
        const nonce = await temp?.web3.eth.getTransactionCount(this.senderPublicKey, "latest") //get latest nonce

        const fun = temp?.nftContract.methods.userMint(to, tokenId, amount, erc721TokenId);
        //the transaction
        let tx = {
            from: this.senderPublicKey,
            to: contractAddress,
