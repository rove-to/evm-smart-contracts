...
contract ArtNFT is ERC1155Tradable {
    ...
    constructor(address admin, address operator, address _parameterAdd, string _name, string _symbol)
    ERC1155Tradable(
        _name,
        _symbol,
        "", admin, operator
    ...
    function userMint(address _to,
        ...
        if (price_tokens[_id] > 0) {
            ...
            uint256 purchaseFeePercent = _p.getUInt256("ART_PUR_FEE");
    ...
    function publishArt(address recipient, uint256 initialSupply, string memory tokenURI, uint256 price, uint256 max)
    external payable
    returns (uint256)
    {
        ParameterControl _p = ParameterControl(parameterControlAdd);
        uint256 publishFee = _p.getUInt256("ART_PUB_FEE");
        if (publishFee > 0) {
